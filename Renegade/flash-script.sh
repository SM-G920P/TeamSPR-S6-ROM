...
# Restore previous modified files if exist
# We do not need to modify framework files anymore
restore_backup /system/framework/core-libart.jar          0    0 0644
restore_backup /system/framework/conscrypt.jar            0    0 0644
...
sed -i "/dalvik.vm.dex2oat-filter/d" /system/build.prop
sed -i "/dalvik.vm.image-dex2oat-filter/d" /system/build.prop
